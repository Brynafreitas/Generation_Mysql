...
select *from tb_produtos order by nome; -- vai aparecer em ordem alfabetica

select *from tb_produtos order by nome DESC; -- vai aparecer em ordem decrescente

select *from tb_produtos where preco >20.00 and categoria_id = 1; -- apenas categoria 1 vai aparecer

select *from tb_produtos where preco >20.00  or categoria_id =1; -- vai trazer todos produtos acima de 20 reais e na categoria 1

select *from tb_produtos where NOT categoria_id =1; -- não vai trazer a categoria 1

select *from tb_produtos where nome like "%ha"; -- vai pegar todos produtos que terminam com ha

select *from tb_produtos where preco in (20,30,40); -- todos produtos com preço  20,30,40

select *from tb_produtos where preco between 20 and 40; -- produtos entre 20 e 40

select *from tb_produtos where dtvalidade between '2021-11-07' and  '2021-11-15' order by dtvalidade, nome;

select count(categoria_id) from tb_produto; -- ira dar total de linhas na categoria id

select sum(preco) from tb_produtos; -- somar todos produtos

select avg(preco) as preco_medio from tb_produtos; -- ira calcular a media as da o apelido

select categoria_id, avg(preco) from tb_produtos group by categoria_id; -- ira calcular as medias por categoria

select max(preco) from tb_produtos; -- vai trazer o valor mais caro

select min(preco) from tb_produtos; -- vai trazer o valor mais barato

select * from tb_produtos where preco = (select max(preco) from tb_produtos); -- vai trazer o valor mais caro e os dados dele
